"""Settings do order-processor: reaproveita pedidos_shared.Settings, valida o que este usa."""
...
def _validate(settings: Settings) -> Settings:
    ...
    if missing:
        raise ValueError(
            f"order-processor requer as seguintes variáveis de ambiente, ausentes: {missing}"
        )
    return settings
